...
export async function getSession(phone) {
  try {
   ...
   if (!data) return { 
  step: 1, greeted: false, started: false, 
  data: {}, completed: false, 
  last_activity: null, awaiting_resume: false, reminder_count: 0 
}

return {
  step:            data.step            || 1,
  greeted:         data.greeted         || false,
  started:         data.started         || false,
  data:            data.data            || {},
  completed:       data.completed       || false,
  last_activity:   data.last_activity   || null,
  awaiting_resume: data.awaiting_resume || false,
  reminder_count:  data.reminder_count  || 0
}
  } catch (err) {
    console.error('getSession failed:', err.message)
    return { step: 1, greeted: false, started: false, data: {}, completed: false }
  }
}
